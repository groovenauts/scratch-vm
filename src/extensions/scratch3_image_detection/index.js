 ...
 * @constructor
 */
class Scratch3ImageDetectionBlocks {
    constructor (runtime) {
        /**
        ...
        /**
         * The truncated MobileNet model
         */
        this.mobilenet = null;

        /**
         * The head model from user specified key
         */
        this.headNet = null;
    ...
    _loop() {
        if (this.headNet == null || this.globalVideoState == "off") {
            this.topLabelsAndProbs = null;
            return;
        }
        ...
        if (offset > 100) {
            ...
            if (frame) {
                const logits = tf.tidy(() => {
                    ...
                    embedding = this.mobilenet.predict(batched);
    ...
    getInfo () {
        ...
        return {
            ...
            name: formatMessage({
                ...
                description: 'Label for the image detection extension category'
            }),
     ...
     */
    getPrediction () {
        if (this.headNet && this.topLabelsAndProbs) {
          return this.topLabelsAndProbs[0].index + 1;
        } else {
          return 0;
        }
    }
    ...
    getTopk(logits) {
        ...
        for (i in logits) {
            valuesAndIndices.push({ value: logits[i], index: Number(i)})
        }
        valuesAndIndices.sort((a, b) => { return b.value - a.value })
        const topLabelsAndProbs = [];
        for (let i = 0; i < MAX_LABELS; i++){
            topLabelsAndProbs.push({ index: valuesAndIndices[i].index, prob: valuesAndIndices[i].value });
        }
        return topLabelsAndProbs;
    }
}

module.exports = Scratch3ImageDetectionBlocks;
